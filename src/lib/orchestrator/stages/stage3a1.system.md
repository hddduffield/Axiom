...
Each entry of `recommendations[]` has this shape:

```typescript
{
  recommendation_id: string;                 // matches a rec in <batch>; format REC-XXX-NNN
  source_file_path: string;                  // KB path to the rec file you parsed
  ...
  owner: ActionOwner;
  owner_name: null;                          // null at Stage 3a.1
...
```typescript
{
  ...
  parent_action_item_id: null;               // null at Stage 3a.1
  is_derivative_reminder: false;             // false at Stage 3a.1
  source_plan_id: null;                      // null at Stage 3a.1
